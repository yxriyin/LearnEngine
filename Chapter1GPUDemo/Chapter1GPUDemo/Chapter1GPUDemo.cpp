...
namespace
{
	...
	{
		{-15.0f,  15.0f,  0.0f, 0xff000000 },
		{ 15.0f,  15.0f,  0.0f, 0xff0000ff },
		{-15.0f, -15.0f,  0.0f, 0xff00ff00 },
		{ 15.0f, -15.0f,  0.0f, 0xff00ffff },
	};

	static const uint16_t s_rectTriList[] =
	{
		0, 1, 2, // 0
		1, 3, 2,
	};
	...
	class Chapter1GPUDemo : public entry::AppI
	{
	public:
		Chapter1GPUDemo(const char* _name, const char* _description, const char* _url)
		...
		void init(int32_t _argc, const char* const* _argv, uint32_t _width, uint32_t _height) override
		{
			...
			m_vbh = bgfx::createVertexBuffer(
				...
				bgfx::makeRef(s_RectVertices, sizeof(s_RectVertices))
			...
			m_ibh[0] = bgfx::createIndexBuffer(
				// Static data can be passed with bgfx::makeRef
				bgfx::makeRef(s_rectTriList, sizeof(s_rectTriList))
			);
			/*
		...
		bool update() override
		{
			if (!entry::processEvents(m_width, m_height, m_debug, m_reset, &m_mouseState))
			{
				...
				for (uint32_t yy = 0; yy < 1; ++yy)
				{
					...
					{
						float mtx[16];
						bx::mtxRotateXY(mtx, 3.14f, 0.0f);
						mtx[12] = 0.0f + float(xx)*3.0f;
						mtx[13] = 0.0f + float(yy)*3.0f;
						mtx[14] = 0.0f;
